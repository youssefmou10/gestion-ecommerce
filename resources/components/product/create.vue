...
<template>
 ...
 <div class="row">
  <router-link to="/allproduct" class="btn btn-primary">All Product</router-link>
    ...
    <div class="row justify-content-center">
     ...
     <div class="col-md-6">
         ...
         <small class="text-danger" v-if="errors.product_code"> {{ errors.product_codel[0] }} </small>
            </div>     
  ...
  <select class="form-control" id="exampleFormControlSelect1" v-model="form.category_id" >
      <option :value="category.id" v-for="category in categories" v-bind:key="category">{{ category.categorie_name}}</option> 
  </select>   
     ...
     <div class="col-md-6">
            <label for="exampleFormControlSelect1">Product Supplier</label>
                      <select class="form-control" id="exampleFormControlSelect1"  v-model="form.supplier_id"  >
                        <option :value="supplier.id" v-for="supplier in suppliers" v-bind:key="supplier" >{{ supplier.name }}</option>
                     </select>
  ...
  <small class="text-danger" v-if="errors.buying_date"> {{ errors.buying_date[0] }} </small>
     ...
     <div class="col-md-6">
        <label for="exampleFormControlSelect1">Product Quantity</label>
         <input type="text" class="form-control" id="exampleInputFirstName" v-model="form.product_quantié" placeholder="enter Your quantite">
         <small class="text-danger" v-if="errors.product_quantié" > {{ errors.product_quantié[0] }} </small>
            </div>     
